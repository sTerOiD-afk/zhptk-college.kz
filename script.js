...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        // ГЛАВНОЕ ИЗМЕНЕНИЕ — открытие нового окна
...
window.openM = function(title, desc, time) {
    ...
    document.getElementById('modalSpec').style.display =
        'flex';
...
window.botSend = function() {
    ...
    const inp =
        document.getElementById('botInp');
    ...
    let res =
        "Не понял вопрос. Попробуйте: 'цена', 'документы', 'реквизиты' или 'кодекс'.";

    if (val.includes("цена") || val.includes("сколько"))
        res =
            "Очное: 140 000 ₸, Заочное: 90 000 ₸. Предоплата 50% до 10 сентября.";

    if (val.includes("док"))
        res =
            "Нужны: Аттестат (оригинал), копия удостоверения, медсправка 075-У, 4 фото 3х4.";

    if (val.includes("реквизит"))
        res =
            "БИН 990140001302, ИИК KZ876017171000000065, АО 'Народный Банк Казахстана'.";

    if (val.includes("кодекс") || val.includes("чест"))
        res =
            "Студенты обязаны соблюдать дисциплину, деловой стиль одежды, уважать преподавателей и вести ЗОЖ.";

    if (val.includes("адрес") || val.includes("где"))
        res =
            "Область Ұлытау, г. Жезказган. Подробности на zhptk.edu.kz.";
